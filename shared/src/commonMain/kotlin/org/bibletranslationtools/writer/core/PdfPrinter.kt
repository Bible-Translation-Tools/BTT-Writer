...
import btt_writer.shared.generated.resources.artwork_attribution_pdf
import btt_writer.shared.generated.resources.label_chapter_title_detailed
import btt_writer.shared.generated.resources.license_pdf
import btt_writer.shared.generated.resources.table_of_contents
...
class PdfPrinter(
    ...
    private suspend fun addTOC(document: Document) {
        document.newPage()
        document.resetPageCount() // disable page numbering for this page (TOC)

        val toc = getString(Res.string.table_of_contents)
    ...
    private suspend fun chapterTitle(c: ChapterTranslation): String {
        val title: String
        if (c.title.isEmpty()) {
            val chapterNumber = Util.strToInt(c.id, 0)
            title = if (chapterNumber > 0) {
                getString(
                    Res.string.label_chapter_title_detailed,
                    "" + chapterNumber
                )
            } else {
    ...
    @Throws(DocumentException::class)
    private suspend fun addChapterPage(document: Document, c: ChapterTranslation) {
        // title
        val title = chapterTitle(c)
    ...
    private suspend fun addContent(document: Document) {
...
//        var progress = 0.0
     ...
     */
    @Throws(DocumentException::class)
    private suspend fun addLicensePage(document: Document) {
        // title
        val title = ""
        ...
        var license = getString(Res.string.license_pdf)

        if (includeMedia) {
            license += getString(Res.string.artwork_attribution_pdf)
        }
