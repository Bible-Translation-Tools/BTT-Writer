package org.bibletranslationtools.writer.integration.rendering
...
class ComposeTextAdapterTest {
    ...
    fun paragraph_indented_produces_newline_plus_spaces() {
        val nodes = listOf(RenderNode.Text("a"), RenderNode.Paragraph(indented = true, children = emptyList()))
        val result = ComposeTextAdapter.convert(nodes)
        assertEquals("a\n    ", result.text)
    }
    ...
    fun paragraph_unindented_produces_newline() {
        val nodes = listOf(RenderNode.Text("a"), RenderNode.Paragraph(indented = false, children = emptyList()))
        val result = ComposeTextAdapter.convert(nodes)
        assertEquals("a\n", result.text)
    }
