...
class TranslationProgressTest {
    ...
    fun `test complete progress with selected source`() {
        ...
        every { preference.getSelectedSourceTranslationId(any()) }
            .returns("id_mrk_ayt")
        every { targetTranslation.numFinished }.returns(8)

        val progress = TranslationProgress(catalogClient, preference).execute(targetTranslation)
        ...
        verify { preference.getSelectedSourceTranslationId(any()) }
    }
