package org.bibletranslationtools.writer.unit.rendering

import org.bibletranslationtools.writer.rendering.USXRenderer
...
class USXRendererTest {
    ...
    fun `empty input returns empty list`() {
        ...
        assertTrue(
            nodes.isEmpty() || nodes.all { it is RenderNode.Text && it.content.isBlank() }
        )
    }
    ...
    fun `whitespace-only input is trimmed to empty`() {
        val nodes = testRender("   \n   ")
        val hasNonBlankText = nodes.any { it is RenderNode.Text && it.content.isNotBlank() }
    ...
    fun `suppressed leading major section heading is excluded`() {
        ...
        assertFalse(
            ...
            nodes.any { it is RenderNode.Section && it.isMajor }
        )
    }
