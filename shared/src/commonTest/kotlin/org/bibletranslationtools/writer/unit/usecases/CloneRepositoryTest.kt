...
class CloneRepositoryTest {
    ...
    @Test
    fun `test clone repository successfully`() = runTest {
        every { repoDir.name }.returns("temp_dir")
        every { repoDir.path }.returns("/temp_dir")
        coEvery { directoryProvider.createTempDir(any()) }.returns(repoDir)
        ...
        val result = CloneRepository(directoryProvider, transportCallback)
    ...
    @Test
    fun `test clone repository with Auth failure exception`() = runTest {
        every { repoDir.name }.returns("temp_dir")
        every { repoDir.path }.returns("/temp_dir")
        coEvery { directoryProvider.createTempDir(any()) }.returns(repoDir)
    ...
    @Test
    fun `test clone private repository throws exception`() = runTest {
        every { repoDir.name }.returns("temp_dir")
        every { repoDir.path }.returns("/temp_dir")
        coEvery { directoryProvider.createTempDir(any()) }.returns(repoDir)
    ...
    @Test
    fun `test no remote repository throws exception`() = runTest {
        every { repoDir.name }.returns("temp_dir")
        every { repoDir.path }.returns("/temp_dir")
        coEvery { directoryProvider.createTempDir(any()) }.returns(repoDir)
    ...
    @Test
    fun `test out of memory throws exception`() = runTest {
        every { repoDir.name }.returns("temp_dir")
        every { repoDir.path }.returns("/temp_dir")
        coEvery { directoryProvider.createTempDir(any()) }.returns(repoDir)
    ...
    @Test
    fun `test generic Throwable exception`() = runTest {
        every { repoDir.name }.returns("temp_dir")
        every { repoDir.path }.returns("/temp_dir")
        coEvery { directoryProvider.createTempDir(any()) }.returns(repoDir)
    ...
    @Test
    fun `test generic Exception exception`() = runTest {
        every { repoDir.name }.returns("temp_dir")
        every { repoDir.path }.returns("/temp_dir")
        coEvery { directoryProvider.createTempDir(any()) }.returns(repoDir)
