...
import org.bibletranslationtools.writer.core.manifest.ManifestAccessor
import org.bibletranslationtools.writer.core.manifest.buildManifest
import org.bibletranslationtools.writer.core.manifest.toSource
...
import org.bibletranslationtools.writer.utils.NumericStringComparator
...
class TargetTranslation private constructor(
    ...
    val manifestAccessor = ManifestAccessor(
        File(path, Manifest.MANIFEST_FILE)
    )
    ...
    private fun readTranslationFormat(): TranslationFormat {
        val parsedFormat = TranslationFormat.Companion.get(manifest.format)
    ...
    @Throws(Exception::class)
    suspend fun merge(newDir: File, onError: (suspend () -> Unit)?): Boolean {
        ...
        val importedManifest = ManifestAccessor(
            File(newDir, Manifest.MANIFEST_FILE)
        ).manifest
        val myRepo = repo
        val git = myRepo.git
    ...
    companion object {
        ...
        suspend fun create(
            ...
            val manifestAccessor = ManifestAccessor(
                File(targetTranslationDir, Manifest.MANIFEST_FILE)
            )
