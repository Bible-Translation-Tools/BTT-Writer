...
class Door43Client @Throws(IOException::class) constructor(
    ...
    private val api: org.unfoldingword.door43client.API

    /**
     * The (mostly) read only index
     */
    val index: org.unfoldingword.door43client.Index

    init {
        // load schema
        if (_root_ide_package_.org.unfoldingword.door43client.Door43Client.Companion.schema == null) {
            _root_ide_package_.org.unfoldingword.door43client.Door43Client.Companion.schema = context.assets.open("schema.sqlite").bufferedReader().use { it.readText() }
        }

        this.api = _root_ide_package_.org.unfoldingword.door43client.API(
            context,
            _root_ide_package_.org.unfoldingword.door43client.Door43Client.Companion.schema!!,
     ...
     * @param listener
     */
    fun setLogger(listener: org.unfoldingword.door43client.OnLogListener?) {
    ...
    suspend fun updateSources(url: String, listener: org.unfoldingword.door43client.OnProgressListener?) {
     ...
     */
    @Throws(Exception::class)
    suspend fun updateCatalogs(force: Boolean, listener: org.unfoldingword.door43client.OnProgressListener?) {
     ...
     */
    @Throws(Exception::class)
    suspend fun updateChunks(listener: org.unfoldingword.door43client.OnProgressListener?) {
