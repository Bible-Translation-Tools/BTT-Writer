...
import models.CategoryEntry
import models.ChunkMarker
import models.Question
import models.Questionnaire
import models.Translation
import models.Versification
...
internal class Library @Throws(IOException::class) constructor(
        ...
        if (db.version == 0) {
            throw IOException(
                "Invalid database version. You probably manually generated the database " +
                        "and forgot to set the \"User Version\" to ${_root_ide_package_.SQLiteHelper.DATABASE_VERSION}"
            )
        }
    }
    ...
    private fun insertOrIgnore(table: String, values: ContentValues, uniqueColumns: Array<String>): InsertResult {
        ...
        var id: Long = -1
        val where = _root_ide_package_.WhereClause.prepare(values, uniqueColumns)
    ...
    @Throws(Exception::class)
    override fun addSourceLanguage(language: models.SourceLanguage): Long {
    ...
    @Throws(Exception::class)
    override fun addTargetLanguage(language: models.TargetLanguage): Boolean {
        validateNotEmpty(language.slug)
        validateNotEmpty(language.name)
        validateNotEmpty(language.direction)
    ...
    @Throws(Exception::class)
    override fun addTempTargetLanguage(language: models.TargetLanguage): Boolean {
        validateNotEmpty(language.slug)
        validateNotEmpty(language.name)
        validateNotEmpty(language.direction)
    ...
    @Throws(Exception::class)
    override fun addProject(project: Project, categories: List<models.Category>?, sourceLanguageId: Long): Long {
    ...
    @Throws(Exception::class)
    override fun addVersification(versification: models.Versification, sourceLanguageId: Long): Long {
    ...
    @Throws(Exception::class)
    fun addChunkMarker(chunk: models.ChunkMarker, projectSlug: String, versificationId: Long): Long {
    ...
    @Throws(Exception::class)
    override fun addCatalog(catalog: models.Catalog): Long {
    ...
    override fun addResource(resource: Resource, projectId: Long): Long {
        ...
        val legacyUrl = resource._legacyData[_root_ide_package_.API.LEGACY_WORDS_ASSIGNMENTS_URL] as? String
    ...
    @Throws(Exception::class)
    fun addQuestionnaire(questionnaire: models.Questionnaire): Long {
    ...
    @Throws(Exception::class)
    fun addQuestion(question: models.Question, questionnaireId: Long): Long {
    ...
    override fun getTranslation(containerSlug: String): models.Translation? {
        return try {
            ...
            if (l != null && p != null && r != null) _root_ide_package_.models.Translation(
                l,
                p,
                r
            ) else null
    ...
    override fun findTranslations(
        ...
        val results = mutableListOf<models.Translation>()
    ...
    override fun getImportedTranslations(): List<models.Translation> {
        val query = """
            select l.slug as language_slug, l.name as language_name, l.direction,
            p.slug as project_slug, p.name as project_name, p.`desc`, p.icon, p.sort, p.chunks_url,
            r.id as resource_id, r.slug as resource_slug, r.name as resource_name, r.type, r.translate_mode, r.checking_level, r.comments, r.pub_date, r.license, r.version,
    ...
    private fun buildTranslation(reader: CursorReader): models.Translation {
        ...
        val r = Resource(
            ...
            _legacyData[_root_ide_package_.API.LEGACY_WORDS_ASSIGNMENTS_URL] = reader.getString("translation_words_assignments_url")
        }
        ...
        return _root_ide_package_.models.Translation(l, p, r)
    }

    override fun getSourceLanguage(sourceLanguageSlug: String): models.SourceLanguage? {
        db.rawQuery("select * from source_language where slug=? limit 1", arrayOf(sourceLanguageSlug)).use { cursor ->
            if (cursor.moveToFirst()) {
                val reader = CursorReader(cursor)
                return _root_ide_package_.models.SourceLanguage(
                    sourceLanguageSlug,
    ...
    override fun getSourceLanguages(): List<models.SourceLanguage> {
        val results = mutableListOf<models.SourceLanguage>()
        db.rawQuery("select * from source_language order by slug asc", null).use { cursor ->
            val reader = CursorReader(cursor)
            while (cursor.moveToNext()) {
                results.add(
                    _root_ide_package_.models.SourceLanguage(
    ...
    override fun getSourceLanguages(projectSlug: String): List<models.SourceLanguage> {
        val query = """
    ...
    override fun getTargetLanguage(targetLanguageSlug: String): models.TargetLanguage? {
        val query = """
            select * from (
        ...
        db.rawQuery(query, arrayOf(targetLanguageSlug)).use { cursor ->
            if (cursor.moveToFirst()) {
                val reader = CursorReader(cursor)
                return _root_ide_package_.models.TargetLanguage(
                    targetLanguageSlug,
                    reader.getString("name"),
                    reader.getString("anglicized_name"),
                    reader.getString("direction"),
                    reader.getString("region"),
                    reader.getBoolean("is_gateway_language")
                )
            }
        }
        return null
    ...
    override fun findTargetLanguage(nameQuery: String): List<models.TargetLanguage> {
        val query = """
            select * from (
              select slug, name, anglicized_name, direction, region, is_gateway_language from target_language
              union
        ...
        val results = mutableListOf<models.TargetLanguage>()
        db.rawQuery(query, arrayOf("%${nameQuery.lowercase()}%")).use { cursor ->
            val reader = CursorReader(cursor)
            while (cursor.moveToNext()) {
                results.add(
                    _root_ide_package_.models.TargetLanguage(
                        reader.getString("slug"),
                        reader.getString("name"),
                        reader.getString("anglicized_name"),
                        reader.getString("direction"),
    ...
    override fun getTargetLanguages(): List<models.TargetLanguage> {
        val query = """
            select * from (
              select slug, name, anglicized_name, direction, region, is_gateway_language from target_language
              union
    ...
    override fun getApprovedTargetLanguage(tempTargetLanguageSlug: String): models.TargetLanguage? {
        val query = """
    ...
    override fun getProjectCategories(
        ...
        languageSlug: String,
        translateMode: String?
    ): List<models.CategoryEntry> {
        val mode = translateMode ?: ""
        val preferredSlugs = arrayOf(languageSlug, "en", "%")
        val projectCategories = mutableListOf<models.CategoryEntry>()
                    ...
                    db.rawQuery(
                        ...
                        if (nameCursor.moveToFirst()) {
                            ...
                            projectCategories.add(
                                _root_ide_package_.models.CategoryEntry(
                                    _root_ide_package_.models.CategoryEntry.Type.CATEGORY,
                                    catId.toLong(),
                                    catSlug,
                                    reader.getString("name"),
                                    reader.getString("source_language_slug"),
                                    parentCategoryId
                                )
                            )
                            found = true
                ...
                for (slug in preferredSlugs) {
                    ...
                    db.rawQuery(
                        ...
                        if (nameCursor.moveToFirst()) {
                            ...
                            projectCategories.add(
                                _root_ide_package_.models.CategoryEntry(
                                    _root_ide_package_.models.CategoryEntry.Type.PROJECT,
                                    projectId,
                                    projectSlug,
                                    reader.getString("name"),
                                    reader.getString("source_language_slug"),
    ...
    override fun getCatalog(catalogSlug: String): models.Catalog? {
        db.rawQuery("select id, url, modified_at from catalog where slug=?", arrayOf(catalogSlug)).use { cursor ->
            if (cursor.moveToFirst()) {
                val reader = CursorReader(cursor)
                return _root_ide_package_.models.Catalog(
                    catalogSlug,
                    reader.getString("url"),
                    reader.getInt("modified_at")
                )
            }
        }
        return null
    ...
    override fun getCatalogs(): List<models.Catalog> {
        val results = mutableListOf<models.Catalog>()
        db.rawQuery("select * from catalog", null).use { cursor ->
            val reader = CursorReader(cursor)
            while (cursor.moveToNext()) {
                results.add(
                    _root_ide_package_.models.Catalog(
                        reader.getString(
                            "slug"
                        ), reader.getString("url"), reader.getInt("modified_at")
                    )
                )
            }
        }
    ...
    override fun getVersification(sourceLanguageSlug: String, versificationSlug: String): models.Versification? {
        val query = """
        ...
        db.rawQuery(query, arrayOf(sourceLanguageSlug, versificationSlug)).use { cursor ->
            if (cursor.moveToFirst()) {
                val reader = CursorReader(cursor)
                return _root_ide_package_.models.Versification(
                    reader.getString(
                        "slug"
                    ), reader.getString("name")
                ).apply {
    ...
    override fun getVersifications(sourceLanguageSlug: String): List<models.Versification> {
        val query = """
        ...
        val results = mutableListOf<models.Versification>()
        db.rawQuery(query, arrayOf(sourceLanguageSlug)).use { cursor ->
            val reader = CursorReader(cursor)
            while (cursor.moveToNext()) {
                results.add(
                    _root_ide_package_.models.Versification(
                        reader.getString(
                            "slug"
                        ), reader.getString("name")
                    ).apply {
    ...
    override fun getChunkMarkers(projectSlug: String, versificationSlug: String): List<models.ChunkMarker> {
        val query = """
        ...
        val results = mutableListOf<models.ChunkMarker>()
        db.rawQuery(query, arrayOf(versificationSlug, projectSlug)).use { cursor ->
            val reader = CursorReader(cursor)
            while (cursor.moveToNext()) {
                results.add(
                    _root_ide_package_.models.ChunkMarker(
                        reader.getString("chapter"),
                        reader.getString("verse")
                    )
                )
            }
        }
    ...
    override fun getQuestionnaire(tdId: Long): models.Questionnaire? {
        db.rawQuery("select * from questionnaire where td_id=$tdId", null).use { cursor ->
            if (cursor.moveToFirst()) {
                ...
                return _root_ide_package_.models.Questionnaire(
    ...
    override fun getQuestionnaires(): List<models.Questionnaire> {
        val results = mutableListOf<models.Questionnaire>()
        db.rawQuery("select * from questionnaire", null).use { cursor ->
            ...
            while (cursor.moveToNext()) {
                ...
                results.add(
                    _root_ide_package_.models.Questionnaire(
                        reader.getString("language_slug"), reader.getString("language_name"),
                        reader.getString("language_direction"), reader.getLong("td_id"), dataFields
                    )
                )
    ...
    override fun getQuestions(questionnaireTDId: Long): List<models.Question> {
        val query = """
        ...
        val results = mutableListOf<models.Question>()
        db.rawQuery(query, null).use { cursor ->
            val reader = CursorReader(cursor)
            while (cursor.moveToNext()) {
                results.add(
                    _root_ide_package_.models.Question(
                        reader.getString("text"),
                        reader.getString("help"),
                        reader.getBoolean("is_required"),
                        _root_ide_package_.models.Question.InputType.get(
                            reader.getString("input_type")
                        ),
                        reader.getInt("sort"),
                        reader.getLong("depends_on"),
    ...
    override fun getCategory(languageSlug: String, slug: String): models.Category? {
        val query = """
        ...
        db.rawQuery(query, arrayOf(slug, languageSlug)).use { cursor ->
            if (cursor.moveToFirst()) {
                val reader = CursorReader(cursor)
                return _root_ide_package_.models.Category(
                    reader.getString(
                        "slug"
                    ), reader.getString("name")
                )
    ...
    private fun getParentCategory(languageSlug: String, childCategorySlug: String): models.Category? {
        val query = """
    ...
    override fun getCategories(languageSlug: String, projectSlug: String): List<models.Category> {
        val categories = mutableListOf<models.Category>()
        val query = """
        ...
        db.rawQuery(query, arrayOf(projectSlug, languageSlug)).use { cursor ->
            if (cursor.moveToFirst()) {
                val reader = CursorReader(cursor)
                val cat = _root_ide_package_.models.Category(
                    reader.getString("slug"), reader.getString("name")
                )
                categories.add(cat)
