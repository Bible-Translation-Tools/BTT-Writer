...
- New translation → `aaa` → `bible-nt` → `John` → type step `Choose a type`: asserts `Unlocked Literal Bible`/`Unlocked Dynamic Bible`/`Notes`/`Questions` visible, `Regular` absent → creates ulb → home shows `John` with `Text ulb` in the Type column
- Same path again → `Notes` → home shows the tn card (`Notes` in the Type column; enabled because the ulb text translation exists)
- New translation → `aaa` → `translationWords` (project only listed in GL mode, creates directly, no type step) → home shows `translationWords` with `Words` in the Type column
- Opens the tn project by tapping its `Notes` type cell, picks the English ULB source, asserts `Read Mode`/`Chunk Mode` sidebar buttons are absent (helps projects are review-only)
